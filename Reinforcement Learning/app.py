...
from evolution import Evolution, Entity  # Import the Evolution and Entity classes
...
class App:
    def __init__(self, settings):
        ...
        self.window.set_caption("REINFORCEMENT LEARNING RACING")
        ...
        if symbol == key.S:
            ...
            if self.entity:
                ...
                if filename:
                    ...
                    show_message(f"Successfully saved {filename} to /{directory}")
            else:
                show_error("No agent to save yet.")
                print(f"Cannot save.")
        # TODO: load file
        if symbol == key.T:
            self.change_track(
    ...
    def on_draw(self):
        ...
        if self.debugging_mode:
            ...
            self.graphics.highlight_car(self.camera_selected_car)
    ...
    # create new generation from best agents
    def new_generation(self):
        self.graphics.clear_batch()

        results = self.simulation.get_agents_results()
        self.simulation.generate_cars_from_agents(
            agents=self.evolution.get_new_generation_from_results(
                results,
        ...
        self.entity.set_agent_from_result(self.evolution.find_best_result(results))
    ...
    def update_labels(self, entity: Entity):
        self.graphics.hud.labels["name"].text = entity.name[:10]  # first 10 characters to fit screen
        self.graphics.hud.labels["gen"].text = "Generation: " + str(int(entity.gen_count))
        self.graphics.hud.labels["max"].text = "Best score: " + str(entity.max_score)
    ...
    # start of simulation
    def start_simulation(self, entity: Entity, track: Track = None, track_filename=None):
        # entity
        self.entity = entity
        self.entity.increment_gen_count()

        # set track or generate random
        if track_filename and os.path.exists(track_filename):
            self.simulation.track = self.tile_manager.load_saved_track(track_filename)
        else:
            self.simulation.track = self.tile_manager.generate_track(shape=(5, 3))
            # optionally, save the generated track to a file
            track_filename = 'track.pkl.npz'
            self.tile_manager.save_track(self.simulation.track, track_filename)

        # set labels
        self.update_labels(self.entity)

        # Generate the initial population of agents
        agents = self.evolution.load_generation(entity.get_random_agent(), self.settings["population"])
        self.simulation.generate_cars_from_agents(
            agents=agents,
            parameters=self.entity.get_car_parameters(),
            images=self.graphics.car_images,

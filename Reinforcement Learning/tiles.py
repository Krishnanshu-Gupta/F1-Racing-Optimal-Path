...
class TileManager:
    ...
    def save_track(self, track, filename):
        ...
        bg_filename = f"{os.path.splitext(filename)[0]}_bg.png"
        track.bg.save(bg_filename)

        # Save the track data and the background image file path
        data = {
            'shape': track.shape,
            'nodes': track.nodes,
            'spawn_index': track.spawn_index,
            'bg_file': bg_filename
        }
    ...
    def load_saved_track(self, filename):
        ...
        # bg_file = data['bg_file']
        bg_file = str(data['bg_file'])

        # Load the background image from the file
        bg = pyglet.image.load(bg_file)
